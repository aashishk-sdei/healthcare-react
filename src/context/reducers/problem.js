...
export const problem = (state = initialState, action) => {
    switch (action.type) {
        //DIAGNOSIS
        case "LIST_PROBLEM":
            return { ...state, records: action.data.records, count: action.data.count };
        case "LIST_PROBLEM_LOAD_MORE":
            return { ...state, records: [...state.records, ...action.data.records], count: action.data.count };
        case "PROBLEM_DETAIL":
