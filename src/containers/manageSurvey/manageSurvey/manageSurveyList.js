...
import { list_survey_category, update_survey_category, update_multi_survey_category, delete_survey_category } from '../../../context/actions/survey';
import { pagination, sorting } from '../../../utils/constants';

const ManageSurveyList = ({ history }) => {
  ...
  useEffect(() => {
    dispatch(list_survey_category({ sortBy: sortBy, sortKey: sortKey, search: search, limit: pagination.limit, page: page }));
  }, []);

  let surveyCategoriesList = useSelector(state => state.survey);
  const _handleDelete = (rid) => {
    dispatch(delete_survey_category({ status: 0, recordId: rid }, result => {
  ...
  const _handleToggle = (state, rid) => {
    dispatch(update_survey_category({ status: state ? 1 : 2, recordId: rid }, result => {
      if (result.messageID === 200) {
        _handlePageChange(1);
      }
    }));
  ...
  const _handlePageChange = (activePage) => {
    setPage(activePage);
    dispatch(list_survey_category({ sortBy: sortBy, sortKey: sortKey, search: search, limit: pagination.limit, page: activePage }));
  }
  const _handleSelected = (key, rid) => {
    ...
    dispatch(update_multi_survey_category({ status: newStatus, recordIds: rid }, result => {
      if (result.messageID === 200) {
  ...
  const _handleSearch = async (key) => {
    await setSearch(key);
    dispatch(list_survey_category({ sortBy: sortBy, sortKey: sortKey, search: key, limit: pagination.limit, page: 1 }));
  }
  ...
  return (
    <>
      <TableList
        list={surveyCategoriesList.records}
        count={surveyCategoriesList.count}
