...
import { list_data, update_label, update_multi_label } from '../../context/actions/label';
...
const LabelList = ({ history }) => {
  ...
  let labels = useSelector(state => state.label);

  const _handleDelete = (rid) => {
    dispatch(update_label({ status: 0, recordId: rid }, result => {
      if (result.messageID === 200) {
        let newPage = page;
        if (page > (labels.count - 1 / pagination.limit)) { newPage = newPage - 1; setPage(newPage); }
  ...
  const _handleToggle = (state, rid) => {
    dispatch(update_label({ status: state ? 1 : 2, recordId: rid }, (() => { _handlePageChange(1); })));
  }
  ...
  const _handleSelected = (key, rid) => {
    ...
    dispatch(update_multi_label({ status: newStatus, recordIds: rid }, result => {
      if (result.messageID === 200) {
        _handlePageChange(1);
      }
    }));
